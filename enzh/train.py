...
TRAIN_FOLDER = "/root/qe/data/QE-sent/en-zh/train"
DEV_FOLDER = "/root/qe/data/QE-sent/en-zh/dev"
TEST_FOLDER = "/root/qe/data/test/CCMT2021_QE_EC_test"
...
if monotransquest_config["evaluate_during_training"]:
    ...
    result_test, model_outputs_test, wrong_predictions_test = model.eval_model(dev, pearson_corr=pearson_corr,
                                                                        spearman_corr=spearman_corr,
                                                                        mae=mean_absolute_error)
    predictions, raw_outputs = model.predict(test_sentence_pairs)
    dev['predictions'] = model_outputs
...
dev_test = un_fit(dev_test, 'labels')
dev_test = un_fit(dev_test, 'predictions')
dev.to_csv(os.path.join(TEMP_DIRECTORY, RESULT_FILE), header=True, sep='\t', index=False, encoding='utf-8')
dev.to_csv(os.path.join(TEMP_DIRECTORY, "dev_test.tsv"), header=True, sep='\t', index=False, encoding='utf-8')
...
draw_scatterplot(dev_test, 'labels', 'predictions', os.path.join(TEMP_DIRECTORY, "test.jpg"), "English-Chinese")
...
useed_time = end_time - begin_time
print('use {} s, {}min'.format(useed_time,useed_time/60))